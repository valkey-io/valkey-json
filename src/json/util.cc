...
const char *jsonutil_code_to_message(JsonUtilCode code) {
    switch (code) {
        ...
        case JSONUTIL_NX_XX_CONDITION_NOT_SATISFIED:
            ...
            break;
        case JSONUTIL_JSON_PARSE_ERROR:
            return "SYNTAXERR Failed to parse JSON string due to syntax error";
        case JSONUTIL_NX_XX_SHOULD_BE_MUTUALLY_EXCLUSIVE:
            return "SYNTAXERR Option NX and XX should be mutually exclusive";
        case JSONUTIL_INVALID_JSON_PATH:
            return "SYNTAXERR Invalid JSON path";
        case JSONUTIL_INVALID_MEMBER_NAME:
            return "SYNTAXERR Invalid object member name";
        case JSONUTIL_INVALID_NUMBER:
            return "SYNTAXERR Invalid number";
        case JSONUTIL_INVALID_IDENTIFIER:
            return "SYNTAXERR Invalid identifier";
        case JSONUTIL_INVALID_DOT_SEQUENCE:
            return "SYNTAXERR Invalid dot sequence";
        case JSONUTIL_EMPTY_EXPR_TOKEN:
            return "SYNTAXERR Expression token cannot be empty";
        case JSONUTIL_ARRAY_INDEX_NOT_NUMBER:
            return "SYNTAXERR Array index is not a number";
        case JSONUTIL_STEP_CANNOT_NOT_BE_ZERO:
            return "SYNTAXERR Step in the slice cannot be zero";
        case JSONUTIL_INVALID_USE_OF_WILDCARD:
            return "ERR Invalid use of wildcard";
        case JSONUTIL_JSON_PATH_NOT_EXIST:
            return "NONEXISTENT JSON path does not exist";
        case JSONUTIL_PARENT_ELEMENT_NOT_EXIST:
            return "NONEXISTENT Parent element does not exist";
        case JSONUTIL_DOCUMENT_KEY_NOT_FOUND:
            return "NONEXISTENT Document key does not exist";
        case JSONUTIL_NOT_A_DOCUMENT_KEY:
            return "WRONGTYPE Not a JSON document key";
        case JSONUTIL_FAILED_TO_DELETE_VALUE:
            return "OPFAIL Failed to delete JSON value";
        case JSONUTIL_JSON_ELEMENT_NOT_NUMBER:
            return "WRONGTYPE JSON element is not a number";
        case JSONUTIL_JSON_ELEMENT_NOT_BOOL:
            return "WRONGTYPE JSON element is not a bool";
        case JSONUTIL_JSON_ELEMENT_NOT_STRING:
            return "WRONGTYPE JSON element is not a string";
        case JSONUTIL_JSON_ELEMENT_NOT_OBJECT:
            return "WRONGTYPE JSON element is not an object";
        case JSONUTIL_JSON_ELEMENT_NOT_ARRAY:
            return "WRONGTYPE JSON element is not an array";
        case JSONUTIL_VALUE_NOT_NUMBER:
            return "WRONGTYPE Value is not a number";
        case JSONUTIL_VALUE_NOT_STRING:
            return "WRONGTYPE Value is not a string";
        case JSONUTIL_VALUE_NOT_INTEGER:
            return "WRONGTYPE Value is not an integer";
        case JSONUTIL_PATH_SHOULD_BE_AT_THE_END:
            return "SYNTAXERR Path arguments should be positioned at the end";
        case JSONUTIL_COMMAND_SYNTAX_ERROR:
            return "SYNTAXERR Command syntax error";
        case JSONUTIL_MULTIPLICATION_OVERFLOW:
            return "OVERFLOW Multiplication would overflow";
        case JSONUTIL_ADDITION_OVERFLOW:
            return "OVERFLOW Addition would overflow";
        case JSONUTIL_EMPTY_JSON_OBJECT:
            return "EMPTYVAL Empty JSON object";
        case JSONUTIL_EMPTY_JSON_ARRAY:
            return "EMPTYVAL Empty JSON array";
        case JSONUTIL_INDEX_OUT_OF_ARRAY_BOUNDARIES:
            return "OUTOFBOUNDARIES Array index is out of bounds";
        case JSONUTIL_UNKNOWN_SUBCOMMAND:
            return "SYNTAXERR Unknown subcommand";
        ...
        case JSONUTIL_DOLLAR_CANNOT_APPLY_TO_NON_ROOT:
            return "SYNTAXERR Dollar sign cannot apply to non-root element";
        case JSONUTIL_ALLOCATION_FAILURE:
            return "ERROR Memory Allocation failed ";
        case JSONUTIL_KEY_OPEN_ERROR:
            return "ERROR Unable to open valkeymodule key";
        default:
...
bool jsonutil_is_root_path(const char *json_path) { return !strcmp(json_path, ".") || !strcmp(json_path, "$"); }
