...
size_t json_get_max_document_size() { return config_max_document_size; }

size_t json_get_defrag_threshold() { return config_defrag_threshold; }

size_t json_get_max_path_limit() { return config_max_path_limit; }

size_t json_get_max_parser_recursion_depth() { return config_max_parser_recursion_depth; }

size_t json_get_max_recursive_descent_tokens() { return config_max_recursive_descent_tokens; }

size_t json_get_max_query_string_size() { return config_max_query_string_size; }
...
bool json_is_instrument_enabled_insert() { return instrument_enabled_insert == 1; }
bool json_is_instrument_enabled_update() { return instrument_enabled_update == 1; }
bool json_is_instrument_enabled_delete() { return instrument_enabled_delete == 1; }
bool json_is_instrument_enabled_dump_doc_before() { return instrument_enabled_dump_doc_before == 1; }
bool json_is_instrument_enabled_dump_doc_after() { return instrument_enabled_dump_doc_after == 1; }
bool json_is_instrument_enabled_dump_value_before_delete() { return instrument_enabled_dump_value_before_delete == 1; }

#define REGISTER_BOOL_CONFIG(ctx, name, default_val, privdata, getfn, setfn)                                   \
    {                                                                                                          \
        if (ValkeyModule_RegisterBoolConfig(ctx, name, default_val, VALKEYMODULE_CONFIG_DEFAULT, getfn, setfn, \
                                            nullptr, privdata) == VALKEYMODULE_ERR) {                          \
...
#define REGISTER_NUMERIC_CONFIG(ctx, name, default_val, flag, min, max, privdata, getfn, setfn)               \
    {                                                                                                         \
        if (ValkeyModule_RegisterNumericConfig(ctx, name, default_val, flag, min, max, getfn, setfn, nullptr, \
                                               privdata) == VALKEYMODULE_ERR) {                               \
            ValkeyModule_Log(ctx, "warning", "Failed to register module config \"%s\".", name);               \
            return VALKEYMODULE_ERR;                                                                          \
        }                                                                                                     \
    }
...
STATIC JsonUtilCode verify_doc_key(ValkeyModuleCtx *ctx, ValkeyModuleString *rmKey, ValkeyModuleKey **key,
                                   bool readOnly = false) {
    *key = static_cast<ValkeyModuleKey *>(
        ValkeyModule_OpenKey(ctx, rmKey, readOnly ? VALKEYMODULE_READ : VALKEYMODULE_READ | VALKEYMODULE_WRITE));
 ...
 */
STATIC JsonUtilCode fetch_json(ValkeyModuleCtx *ctx, ValkeyModuleString *rmKey, const char *path, PrintFormat *format,
                               ReplyBuffer &oss) {
    ValkeyModuleKey *key;
...
STATIC JsonUtilCode parseAndValidateMSetCmdArgs(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, const int argc,
                                                MSetCmdArgs **args_list, size_t *num_keys) {
    ...
    *args_list = reinterpret_cast<MSetCmdArgs *>(ValkeyModule_Alloc((*num_keys) * sizeof(MSetCmdArgs)));
    ...
    for (i = 0; i < *num_keys; ++i) {
        ...
        if (type != VALKEYMODULE_KEYTYPE_EMPTY && ValkeyModule_ModuleTypeGetType(current_arg.key) != DocumentType) {
        ...
        if (current_arg.is_root_path) {
            ...
            if (json_is_instrument_enabled_insert() || json_is_instrument_enabled_update()) {
                ...
                ValkeyModule_Log(ctx, "warning",
                                 "Dump document structure before setting JSON key (hashed) %zu whole doc %p:", key_hash,
 ...
 */
STATIC JsonUtilCode parseSimpleCmdArgs(ValkeyModuleString **argv, const int argc, ValkeyModuleString **key,
                                       const char **path) {
    *key = nullptr;
    *path = nullptr;
...
STATIC JsonUtilCode parseNumIncrOrMultByCmdArgs(ValkeyModuleString **argv, const int argc, ValkeyModuleString **key,
                                                const char **path, JValue *jvalue, size_t *jvalue_size) {
    *key = nullptr;
    *path = nullptr;
    *jvalue_size = 0;
...
STATIC JsonUtilCode parseStrAppendCmdArgs(ValkeyModuleString **argv, const int argc, ValkeyModuleString **key,
                                          const char **path, const char **json, size_t *json_len) {
    *key = nullptr;
...
STATIC JsonUtilCode parseArrPopCmdArgs(ValkeyModuleString **argv, const int argc, ValkeyModuleString **key,
                                       const char **path, int64_t *index) {
    *key = nullptr;
 ...
 * @return
 */
STATIC JsonUtilCode parseArrTrimCmdArgs(ValkeyModuleString **argv, const int argc, ValkeyModuleString **key,
                                        const char **path, int64_t *start, int64_t *stop) {
    *key = nullptr;
    *path = nullptr;
    *start = 0;
    *stop = 0;
...
int Command_JsonSet(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    // verify valkey keys
    ValkeyModuleKey *key =
        static_cast<ValkeyModuleKey *>(ValkeyModule_OpenKey(ctx, args.key, VALKEYMODULE_READ | VALKEYMODULE_WRITE));
    ...
    if (is_new_valkey_key) {
        if (!is_root_path) return ValkeyModule_ReplyWithError(ctx, ERRMSG_NEW_VALKEY_KEY_PATH_NOT_ROOT);
        if (args.is_update_only) return ValkeyModule_ReplyWithNull(ctx);
    } else {
        if (is_root_path && args.is_create_only) return ValkeyModule_ReplyWithNull(ctx);
    }
    ...
    if (is_root_path) {  // root doc
        ...
        if (rc != JSONUTIL_SUCCESS) {
            if (rc == JSONUTIL_NX_XX_CONDITION_NOT_SATISFIED) return ValkeyModule_ReplyWithNull(ctx);
            return ValkeyModule_ReplyWithError(ctx, jsonutil_code_to_message(rc));
        }
...
int Command_JsonGet(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    if (num_paths == 0) {
        ...
        const char **cstr_paths =
            static_cast<const char **>(ValkeyModule_PoolAlloc(ctx, num_paths * sizeof(const char *)));
...
int Command_JsonMGet(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    for (int i = 0; i < num_keys; i++) {
        ...
        if (rc != JSONUTIL_SUCCESS && rc != JSONUTIL_DOCUMENT_KEY_NOT_FOUND && rc != JSONUTIL_INVALID_JSON_PATH &&
            rc != JSONUTIL_JSON_PATH_NOT_EXIST) {
            return ValkeyModule_ReplyWithError(ctx, jsonutil_code_to_message(rc));
        }
    }
...
int Command_JsonArrInsert(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    jsn::vector<size_t> vec;
    bool is_v2_path;
    rc = dom_array_insert(ctx, doc, args.path, args.index, args.jsons, args.json_len_arr, args.num_values, vec,
                          is_v2_path);
    if (rc != JSONUTIL_SUCCESS) return ValkeyModule_ReplyWithError(ctx, jsonutil_code_to_message(rc));
...
int Command_JsonArrIndex(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    bool is_v2_path;
    rc = dom_array_index_of(doc, args.path, args.scalar_val, args.scalar_val_len, args.start, args.stop, vec,
                            is_v2_path);
    if (rc != JSONUTIL_SUCCESS) return ValkeyModule_ReplyWithError(ctx, jsonutil_code_to_message(rc));
...
int Command_JsonDebug(ValkeyModuleCtx *ctx, ValkeyModuleString **argv, int argc) {
    ...
    if (!strcasecmp(subcmd, "MEMORY")) {
        ...
        cmds.push_back(
            "JSON.DEBUG MEMORY <key> [path] - report memory size (bytes). "
        ...
        cmds.push_back(
            "JSON.DEBUG FIELDS <key> [path] - report number of fields in the "
...
void *DocumentType_RdbLoad(ValkeyModuleIO *rdb, int encver) {
    if (encver > DOCUMENT_TYPE_ENCODING_VERSION) {
        ValkeyModule_LogIOError(
            rdb, "warning", "Cannot load document type version %d, because current module's document version is %d.",
...
void *DocumentType_Copy(ValkeyModuleString *from_key_name, ValkeyModuleString *to_key_name, const void *value) {
...
#define beginSection(name) if (ValkeyModule_InfoAddSection(ctx, const_cast<char *>(name)) != VALKEYMODULE_ERR) {
#define endSection() }
...
int handleFactor(float KeyTable::Factors::*f, const void *v, const char *name) {
    ...
    if (validity == nullptr) {
        ...
        ValkeyModule_Log(nullptr, "warning", "Error setting parameter %s to %g", validity, double(value));
        return VALKEYMODULE_ERR;
    }
}
...
int handleHashTableFactor(T rapidjson::HashTableFactors::*f, const void *v, T scale_factor) {
    ...
    if (validity == nullptr) {
        ...
        ValkeyModule_Log(nullptr, "warning", "Error setting parameter %s from (unscaled: %d)", validity, unscaled);
        return VALKEYMODULE_ERR;
    }
}
...
int configKeyTable() {
    long long val = 100;
    if (handleFactor(&KeyTable::Factors::grow, &val, "key-table-grow-factor") == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 50;
    if (handleFactor(&KeyTable::Factors::shrink, &val, "key-table-shrink-factor") == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 25;
    if (handleFactor(&KeyTable::Factors::minLoad, &val, "key-table-min-load-factor") == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 85;
    if (handleFactor(&KeyTable::Factors::maxLoad, &val, "key-table-max-load-factor") == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
...
int configHashtable() {
    long long val = 100;
    if (handleHashTableFactor(&rapidjson::HashTableFactors::grow, &val, 100.f) == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 50;
    if (handleHashTableFactor(&rapidjson::HashTableFactors::shrink, &val, 100.f) == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 25;
    if (handleHashTableFactor(&rapidjson::HashTableFactors::minLoad, &val, 100.f) == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
    val = 85;
    if (handleHashTableFactor(&rapidjson::HashTableFactors::maxLoad, &val, 100.f) == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;
...
int registerModuleConfigs(ValkeyModuleCtx *ctx) {
    ...
    REGISTER_NUMERIC_CONFIG(ctx, "max-path-limit", DEFAULT_MAX_PATH_LIMIT, VALKEYMODULE_CONFIG_DEFAULT, 0, INT_MAX,
...
bool set_command_info(ValkeyModuleCtx *ctx, const char *name, int32_t arity, uint64_t keyspec_flags, int bs_index,
                      std::tuple<int, int, int> key_range) {
    // Get command
    ValkeyModuleCommand *command = ValkeyModule_GetCommand(ctx, name);
    if (command == nullptr) {
        ValkeyModule_Log(ctx, "warning", "Failed to get command '%s'", name);
...
extern "C" int ValkeyModule_OnLoad(ValkeyModuleCtx *ctx) {
    ...
    // Create module type
    DocumentType = ValkeyModule_CreateDataType(ctx, DOCUMENT_TYPE_NAME, DOCUMENT_TYPE_ENCODING_VERSION, &type_methods);
    if (DocumentType == nullptr) {
        ValkeyModule_Log(ctx, "warning", "Failed to create data type %s encver %d", DOCUMENT_TYPE_NAME,
                         DOCUMENT_TYPE_ENCODING_VERSION);
        return VALKEYMODULE_ERR;
    }
    ...
    if (ValkeyModule_AddACLCategory(ctx, json_category) == VALKEYMODULE_ERR) return VALKEYMODULE_ERR;
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.SET"), cat_slow_write_deny) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.MSET"), cat_slow_write_deny) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.GET"), cat_readonly) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.MGET"), cat_readonly) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.DEL"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.DEL.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.FORGET", Command_JsonDel, cmdflg_fast_write, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.FORGET.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.FORGET"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.FORGET.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.NUMINCRBY", Command_JsonNumIncrBy, cmdflg_fast_write, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.NUMINCRBY.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.NUMINCRBY"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.NUMINCRBY.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.NUMMULTBY", Command_JsonNumMultBy, cmdflg_fast_write, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.NUMMULTBY.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.NUMMULTBY"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.NUMMULTBY.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.STRLEN", Command_JsonStrLen, cmdflg_readonly, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.STRLEN.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.STRLEN"), cat_readonly) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.STRLEN.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.STRAPPEND", Command_JsonStrAppend, cmdflg_fast_write_deny, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.STRAPPEND.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.STRAPPEND"), cat_fast_write_deny) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.STRAPPEND.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.TOGGLE", Command_JsonToggle, cmdflg_fast_write_deny, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.TOGGLE.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.TOGGLE"), cat_fast_write_deny) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.TOGGLE.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.OBJLEN", Command_JsonObjLen, cmdflg_readonly, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.OBJLEN.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.OBJLEN"), cat_readonly) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.OBJKEYS"), cat_readonly) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.OBJKEYS.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.ARRLEN", Command_JsonArrLen, cmdflg_readonly, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.ARRLEN.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRLEN"), cat_readonly) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.ARRLEN.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.ARRAPPEND", Command_JsonArrAppend, cmdflg_fast_write_deny, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.ARRAPPEND.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRAPPEND"), cat_fast_write_deny) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRPOP"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.ARRPOP.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.ARRINSERT", Command_JsonArrInsert, cmdflg_fast_write_deny, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.ARRINSERT.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRINSERT"), cat_fast_write_deny) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.ARRINSERT.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.ARRTRIM", Command_JsonArrTrim, cmdflg_fast_write, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.ARRTRIM.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRTRIM"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.ARRTRIM.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.CLEAR", Command_JsonClear, cmdflg_fast_write, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.CLEAR.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.CLEAR"), cat_fast_write) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.CLEAR.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.ARRINDEX", Command_JsonArrIndex, cmdflg_readonly, 1, 1, 1) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.ARRINDEX.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.ARRINDEX"), cat_readonly) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.TYPE"), cat_readonly) ==
        VALKEYMODULE_ERR) {
    ...
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.RESP"), cat_readonly) ==
        VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to set command category for JSON.RESP.");
        return VALKEYMODULE_ERR;
    }

    if (ValkeyModule_CreateCommand(ctx, "JSON.DEBUG", nullptr, cmdflg_debug, 0, 0, 0) == VALKEYMODULE_ERR) {
        ValkeyModule_Log(ctx, "warning", "Failed to create command JSON.DEBUG.");
        return VALKEYMODULE_ERR;
    }
    if (ValkeyModule_SetCommandACLCategories(ValkeyModule_GetCommand(ctx, "JSON.DEBUG"), cat_debug) ==
        VALKEYMODULE_ERR) {
