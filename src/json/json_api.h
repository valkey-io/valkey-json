...
using ValkeyModuleCtx = struct ValkeyModuleCtx;
using ValkeyModuleKey = struct ValkeyModuleKey;
using ValkeyModuleString = struct ValkeyModuleString;
 ...
 */
int get_json_value_type(ValkeyModuleCtx *ctx, const char *keyname, const size_t key_len, const char *path, char **type,
                        size_t *len);
 ...
 * @return 0 - success, 1 - error
 */
int get_json_value(ValkeyModuleCtx *ctx, const char *keyname, const size_t key_len, const char *path, char **value,
                   size_t *len);
 ...
 *              The caller is responsible for freeing the memory: the array '*values' as well as all the strings
 * '(*values)[i]'.
 ...
 *              The caller is responsible for freeing the memory: the array '*types' as well as all the strings
 * '(*types)[i]'.
...
int get_json_values_and_types(ValkeyModuleCtx *ctx, const char *keyname, const size_t key_len, const char **paths,
                              const int num_paths, char ***values, size_t **lengths, char ***types,
                              size_t **type_lengths);
